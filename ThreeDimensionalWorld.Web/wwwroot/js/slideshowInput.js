...
fileInput.addEventListener('change', function (event) {
    ...
    slideshowContainer.replaceChildren(slideshowContainer.querySelector(".slideshow-btn-prev"), slideshowContainer.querySelector(".slideshow-btn-next"))

    slideshowDotsContainer.replaceChildren();

    Array.from(fileList).map((item, i) => {
        const span = document.createElement('span');
        span.classList.add("slideshow-dot");
        return span
    ...
    for (const file of fileList) {
        ...
        if (fileExtension == ".stl") {
            ...
            div.classList.add("slideshow-slides");
